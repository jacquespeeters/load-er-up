...
def preprocess(data_file):
    ...
    logger.info(
        f"features after preprocessing has {len(x_inputs)} machines and shape of {x_inputs[0].shape}"
    )
    logger.info(
        f"target after preprocessing has {len(actuals)} machines and shape of {actuals[0].shape}"
    )
...
def generate_actuals(df_machine):
    ...
    return (
        ...
        .groupby("window")[["operating", "performing", "y_0"]]
        .max()
